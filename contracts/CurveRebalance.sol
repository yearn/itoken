...
contract CurveRebalance is ReentrancyGuard, Ownable, Structs {
  ...
  // dydx 0 = ETH, 1 = SAI, 2 = USDC, 3 = DAI
  // curve 0 = DAI, 1 = USDC, 2 = USDT, 3 = TUSD
  ...
  function withdrawTUSD(uint256 amount) public onlyOwner {
    _amount = amount;
    _to = 3; // TUSD
    _from = 1; // USDC
    _token = tusd;
  ...
  function _flash(uint256 amount) internal {
    ...
    withdraw.primaryMarketId = _dydx; // Borrow USDC
    ...
    deposit.accountId = 0;
    deposit.amount = damt;
    deposit.primaryMarketId = _dydx; // Repay USDC
  ...
  function callFunction(
    ...
    ICurveFi(swap).exchange_underlying(_from, _to, _amount, 0);
    ICurveFi(swap).exchange_underlying(_to, _from, IERC20(_token).balanceOf(address(this)), 0);
  }
  ...
  function approveToken() public {
      ...
      IERC20(tusd).safeApprove(swap, uint(-1));
  }
